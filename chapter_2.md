...
那么这样的一个市场是否存在套利机会呢？我们可以看到，通过构建一个套利组合$P=(0.3, 0.2, -0.5)$，可以实现投资总额$\sum_{i=1}^N w_i = 0.3 + 0.2 + (-0.5) = 0$，同时，系统性风险$\sum_{i=1}^N w_i \beta_i = 0.3 * 1 + 0.2 * 3.5 - 0.5 * 2.0 = 0$。但是总的收益$\sum_{i=1}^N w_i r_i = 12\% \times 0.3 + 25\% \times 0.2 + 15\% \times (-0.5) = 1.1\% > 0$。
